...
SELECT * FROM TABLE(
    INFER_SCHEMA(
        LOCATION => '@S3_TASTY_FILES/raw_pos/franchise',
...
USE DATABASE TASTY_BYTES;
USE SCHEMA RAW;
CREATE OR REPLACE TABLE RAW_franchise
(
...
--Load data in the franchise table

COPY INTO TASTY_BYTES.RAW.RAW_franchise
FROM 
(
    SELECT 
        $1,
    ...
    FROM @TASTY_BYTES.PUBLIC.S3_TASTY_FILES/raw_pos/franchise/franchise.csv.gz
)
